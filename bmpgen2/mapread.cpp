...
static LONG OnResStart( HWND hwnd, HWND hwndLB )
{
	...
	while ( *p ) {
		...
		*file = 0;
		...
		if ( !AddArcFile( temp, (add_arc_callback_t)dlg_cb_proc, ListBox_GetCount( hwndLB ), ListBox_GetCount( hwndLB ), hwndLB ) ) {
			switch ( errno ) {
			case ENOMEM:
				MessageBox( hwnd, L"Некорректный файл или недостаточно ресурсов", mapread, MB_OK | MB_ICONEXCLAMATION );
				break;
			case ENOENT:
				swprintf( txt, sizeof(txt)/sizeof(txt[0]), L"Ошибка открытия файла '%s'", (LPSTR)temp );
...
{
	switch ( wmsg ) {
	case WM_USER+1000:   return OnResStart( hwnd, GetDlgItem( hwnd, IDC_RESULTS ) );
...
#define G2G(g,m,s)		((g)+(m)/geo_60+(s)/geo_3600)
	...
	infile = _wfopen( fname, L"rb" );
	if ( !infile ) return 0;
	setvbuf( infile, temp_buf, _IOFBF, sizeof(temp_buf) );
	...
	fgets( line_buf, sizeof(line_buf)/sizeof(line_buf[0]), infile );
	trimline( s = bow( line_buf ) );	/* skip title */

	for ( pts = 0; !feof(infile); pts++ ) {  // enumerate points
		if ( !fgets( line_buf, sizeof(line_buf)/sizeof(line_buf[0]), infile ) ) continue;
	...
	for ( i= 0; !feof(infile); i++ ) {   // enumerate points
		if ( !fgets( line_buf, sizeof(line_buf)/sizeof(line_buf[0]), infile ) ) continue;
		p.latitude = atogeo( s = bow( line_buf ) );
		if ( !*s ) continue;
		...
		case 3: case 13:/* 3 = rhomb */
			{
				...
				pts[0].x = iround(p.x - v*Dmx);	pts[0].y = iround(p.y);
				pts[1].x = iround(p.x);		pts[1].y = iround(p.y - v*Dmy);
				pts[2].x = iround(p.x + v*Dmx);	pts[2].y = iround(p.y);
				pts[3].x = iround(p.x);		pts[3].y = iround(p.y + v*Dmy);
				Polygon( hdc, pts, 4 );
			}
			break;
		case 4: case 14:/* 4 = triangle */
			{
				...
				pts[0].x = iround(p.x - v*Dmx);	pts[0].y = iround(p.y + 2*0.2887*v*Dmy);
				pts[1].x = iround(p.x);		pts[1].y = iround(p.y - 2*0.57735*v*Dmy);
				pts[2].x = iround(p.x + v*Dmx);	pts[2].y = iround(p.y + 2*0.2887*v*Dmy);
				Polygon( hdc, pts, 3 );
			}
			break;
		}
