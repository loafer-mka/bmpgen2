...
Примечание 1: программа была разработана в 90-х для Windows 3.10, далее, примерно в 1997 году, модифицирована
...
Примечание 2: поддерживается режим частичной автоматизации, позволяющий по одному набору контуров и реперных
точек создавать серию изображений. Для этого надо указать список файлов с контурами, с реперными точками, 
задать имя создаваемого растрового изображения, а также задать имя несуществующего файла отметок и отметить
галочкой пункт "wait for input and...". Далее программа переходит в режим ожидания и, как только обнаруживает
существующий файл отметок, создаёт изображение карты и удаляет входной файл отметок.
Файл отметок используется для согласования работы "управляющей" программы и bmpgen2; рекомендуется создавать
временный файл отметок под каким-либо временным именем, затем переименовать его в соответствии с тем, как
указано для bmpgen2 и дождаться его удаления. В тот момент, когда файл отметок удаляется bmpgen2'ом, файл с
изображением уже создан и его можно переименовать или переместить в нужную папку.
